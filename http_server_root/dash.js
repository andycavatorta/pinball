...
function create_group_from_array_of_paths(dom_parent, array_of_paths, path_attributes_o = new Object(), group_attributes_o = new Object()){
    ...
    for (var path_i=0; path_i<array_of_paths.length;path_i++){
...
class Carousel_Panel{
  constructor(dom_parent,coordinates) {
    this.coordinates = coordinates;
    this.dom_parent = dom_parent;
...
class Words{
  constructor(dom_parent, scaling_factor) {
    this.dom_parent = dom_parent;
    var path_strings_como = [
    ...
    var paths_dinero = create_group_from_array_of_paths(
      this.dom_parent, 
      path_strings_dinero, 
      {
        class:"carousel_segment"
    ...
    setAttributes(paths_dinero,{transform:`scale(${scaling_factor},${scaling_factor})translate(0,0)`,});

    var paths_juega = create_group_from_array_of_paths(
      this.dom_parent, 
      path_strings_juega, 
      {
        class:"carousel_segment"
    ...
    setAttributes(paths_juega,{transform:`scale(${scaling_factor},${scaling_factor})translate(0,0)`,});

    var paths_como = create_group_from_array_of_paths(
      this.dom_parent, 
      path_strings_como,
      {
        class:"carousel_segment"
    ...
    setAttributes(paths_como,{transform:`scale(${scaling_factor},${scaling_factor})translate(0,0)`,});

    var paths_fue = create_group_from_array_of_paths(
      this.dom_parent, 
      path_strings_fue, 
      {
        class:"carousel_segment"
    ...
    setAttributes(paths_fue,{transform:`scale(${scaling_factor},${scaling_factor})translate(0,0)`,});

    var paths_trueque = create_group_from_array_of_paths(
      this.dom_parent, 
      path_strings_trueque, 
      {
        class:"carousel_segment"
      },
      {
    ...
    setAttributes(paths_trueque,{transform:`scale(${scaling_factor},${scaling_factor})translate(0,0)`,});
  }
}

class Words_Numerals_Panel{
  // contains one Words and three Numerals
  // contains method to change words 
  // contains method to update numerals as three digit number
  constructor(dom_parent) {
    this.dom_parent = dom_parent;
    this.panel = create_group(this.dom_parent, {});
    this.words = new Words(this.panel, 0.5);
  }
}

class Overhead_Panel{
  // contains five instances of Words_Numerals_Panel
  // contains five instances of Chimes_Panel
  // contains methods for accessing all panels 
  constructor(dom_parent, coordinates) {
    this.dom_parent = dom_parent;
    const words_numerals_width = 200;
    this.panel = create_group(this.dom_parent, {transform:`matrix(1,0,0,1,${coordinates[0]},${coordinates[1]})`});
    var words_numerals_coordinates = [
      [0, -words_numerals_width, 0],
      [-words_numerals_width*Math.sin(degrees_to_radians(72)),-words_numerals_width*Math.cos(degrees_to_radians(72)), -72],
      //[words_numerals_width*Math.sin(degrees_to_radians(72)),-words_numerals_width*Math.cos(degrees_to_radians(72)), -72],
      //[-words_numerals_width*Math.sin(degrees_to_radians(36)),words_numerals_width*Math.cos(degrees_to_radians(36)), -144],
      //[words_numerals_width*Math.sin(degrees_to_radians(36)),words_numerals_width*Math.cos(degrees_to_radians(36)), -72]
    ];
    this.words_numerals_a = []
    for (var c_i in words_numerals_coordinates){
      var words_numerals_instance = new Words_Numerals_Panel(this.panel);
      setAttributes(words_numerals_instance.panel, {
        'transform': `translate(${words_numerals_coordinates[c_i][0]},${words_numerals_coordinates[c_i][1]}),rotate(${words_numerals_coordinates[c_i][2]})`
      });
      this.words_numerals_a.push(words_numerals_instance);
    }
  }
}
...
function init() {
  ...
  interface.carousel_panel = new Carousel_Panel(canvas,[1850,1200]);
  var overhead_panel = new Overhead_Panel(canvas,[2250,600]);
  //d = new Words(canvas, 0.5)
