...
class Fake_Attraction_Mode(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            time.sleep(3)
            self.tb.publish("playfield_lights",["trail_rollover_right","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_rollover_right","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_rollover_left","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_rollover_left","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_sling_right","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_sling_right","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_sling_left","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_sling_left","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_pop_left","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_pop_left","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_pop_right","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_pop_right","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_pop_center","off"], self.gamestation_names[2])
            self.tb.publish("playfield_lights",["pie_pop_center","on"], self.gamestation_names[2])
            time.sleep(0.5)
            self.tb.publish("playfield_lights",["trail_spinner","off"], self.gamestation_names[2])
