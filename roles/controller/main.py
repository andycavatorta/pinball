...
class Fake_Attraction_Mode(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_rollover_right","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_rollover_right","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_rollover_left","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_rollover_left","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_sling_right","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_sling_right","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_sling_left","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_sling_left","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_pop_left","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_pop_left","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_pop_right","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_pop_right","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_pop_center","off"], self.gamestation_names[station_ordinal])
            self.tb.publish("playfield_lights",["pie_pop_center","on"], self.gamestation_names[station_ordinal])
            time.sleep(1)
            self.tb.publish("playfield_lights",["trail_spinner","off"], self.gamestation_names[station_ordinal])
