...
class Multimorphic(threading.Thread):
    ...
    def enable_dinero(self, enable_bool, miliseconds=15):
        if enable_bool:
            ...
            self.p3.clear_rule(self.switches["dinero"])
    def enable_kicker(self, enable_bool, miliseconds=15):
        if enable_bool:
            self.p3.configure_pops_slings(self.switches["kicker"], self.coils["kicker"], miliseconds)
        else:
            self.p3.clear_rule(self.switches["kicker"])
    def enable_izquierda(self, enable_bool, miliseconds=17):
        if enable_bool:
    ...
    def enable_derecha(self, enable_bool, miliseconds=17):
        if enable_bool:
